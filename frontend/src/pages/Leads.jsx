...
/* ── helpers ── */
const EMPTY_FORM = { name: '', email: '', source: 'Website', status: 'New', value: 0 };
...
export default function Leads() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editTarget) {
        await api.updateLead(editTarget._id, formData);
    ...
    <Layout>
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {filtered.map(lead => (
                <tr key={lead._id} className="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-8 py-5 font-black text-slate-900">DH{(lead.value || 0).toLocaleString()}</td>
                  <td className="px-8 py-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm">
          ...
          <h3 className="text-4xl font-black text-slate-900 mb-1">DH{leads.reduce((s, l) => s + (l.value || 0), 0).toLocaleString()}</h3>
