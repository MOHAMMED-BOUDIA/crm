...
export default function Tasks() {
  ...
  const filteredTasks = tasks.filter(t => {
    ...
    if (dueFilter === 'On Track' && t.overdue) return false;
  ...
  return (
    <Layout>
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-8 flex flex-col sm:flex-row items-start sm:items-end gap-4">
          ...
          <div className="w-full sm:w-56">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Due Status</label>
            <select
              className={INPUT_CLS}
              value={dueFilter}
          ...
          <div className="w-full sm:w-auto sm:ml-auto flex gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={clearFilters} className="w-full sm:w-auto">
              Reset
            </Button>
            <Button type="button" variant="secondary" size="sm" onClick={() => setShowFilters(false)} className="w-full sm:w-auto">
              Close
            </Button>
          </div>
        </div>
